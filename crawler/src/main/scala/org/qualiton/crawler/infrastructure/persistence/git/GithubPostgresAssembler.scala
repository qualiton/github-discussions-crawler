...
object GithubPostgresAssembler {
  ...
  def toDomain(discussionPersistence: DiscussionPersistence): Validated[Throwable, Discussion] = {
    ...
    val urlValidated: ValidatedNel[ValidationError, Url] = refineV[RefinedUrl](url).leftMap(ValidationError(_)).toValidatedNel
    ...
    val discussionValidated: ValidatedNel[ValidationError, Discussion] =
      (teamNameValidated, titleValidated, authorValidated, bodyValidated, bodyVersionValidated, urlValidated)
        .mapN((teamName, title, author, body, bodyVersion, url) =>
          Discussion(teamId, teamName, discussionId, title, author, body, bodyVersion, url, List.empty, createdAt, updatedAt))
