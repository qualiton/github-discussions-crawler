...
package core {
  ...
  final case class NewDiscussionDiscoveredEvent(teamName: NonEmptyString,
                                                title: NonEmptyString,
                                                author: NonEmptyString,
                                                url: Url,
                                                totalCommentsCount: Long,
                                                addressees: Set[Addressee],
                                                createdAt: Instant) extends Event

  final case class NewCommentsDiscoveredEvent(teamName: NonEmptyString,
                                              title: NonEmptyString,
                                              totalCommentsCount: Long,
                                              comments: NonEmptyList[NewComment]) extends Event

  final case class NewComment(author: NonEmptyString,
                              url: Url,
                              addressees: Set[Addressee],
                              createdAt: Instant)
